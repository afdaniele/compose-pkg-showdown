...
(function(extension) {
    ...
    function sortHeader(tocEntries, level) {
        ...
        for (var i = 0; i < tocEntries.length; i++) {
            h_lvl = parseInt(tocEntries[i].tagName.charAt(1));
            if( h_lvl < min_level )
                min_level = h_lvl;
        }
        if( min_level == 99 )
    ...
    return [{
        ...
        filter: function(sourceHtml) {
            ...
            while(true){
                next_tos_idx = sourceHtml.indexOf( '[toc]', start );
                if( next_tos_idx == -1 )
                    break
                tocs.push( next_tos_idx );
                start = next_tos_idx + 5;
            }
            tocs.push( sourceHtml.length );

            var output_str = sourceHtml.substring(0, tocs[0]);

            start = 0;
            for (var i = 0; i < tocs.length-1; i++) {
                start = tocs[i]+5;
                end = tocs[i+1];

                source = sourceHtml.substring(start, end);

                var headerList = getHeaderEntries(source);
